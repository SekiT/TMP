...
export const buildDiContainer = (dependencies) => {
  ...
  const container = Object.entries(dependencies).reduce(
    (acc, [packageName, packageInstances]) => {
      const packageHandlers = {};
      const proxies = Object.entries(packageInstances).reduce((acc2, [key, value]) => {
        const handler = {};
        packageHandlers[key] = handler;
        return {
          ...acc2,
          [key]: new Proxy(value, handler),
        };
      }, {});
      handlers[packageName] = packageHandlers;
      return { ...acc, [packageName]: proxies };
    },
    {},
  );
